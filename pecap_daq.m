function pecap_daq(strOutputFilename, Rec_Elec, Gain_dB, Delay_us, ...
    PRate_Hz, sweeps, elecs, e_levels, half)
...
% devices
%
% Cosentino et al (2015) and Garcia et al (2021) describe a method that 
% allows reconstruction of the underlying neural activation pattern of 
% individual channels from ECAP amplitudes, the latter separating these
% activation patterns into separate estimates of current spread and neural
% health along the length of the cochlea. The method utilizes every 
% combination of masker and probe electrode available in the subject's MAP. 
% This software generates .csv files that can be used to gather the data 
% required for PECAP as imports to Custom Sound EP Software. It is written 
% in MATLAB and should run on any platform supporting that language, 
% although it has only been evaluated on Windows 10 using MATLAB R2018a and
% Custom Sound EP 5.0
%
% If you use this software, please cite the following publication that
% contains details of how the updated PECAP2 method words:
...
% pecap_daq(strOutputFilename, Rec_Elec, Gain_dB, Delay_us, ...
%       PRate_Hz, sweeps, elecs, e_levels, half)
%
...
%   the probe or the masker electrode. The function will not overwrite an
%   existing file if named redundantly.
%
%   Inputs:
%       - strOutputFilename:    Name of the csv file the script will create
%                               i.e. 'Subject1' will create two files:
%                               'Subject1_verification.csv' and 
%                               'Subject1.csv'
%       - Rec_Elec:     number that records the default offset of the
%                       recording electrode from the probe, +2 (basal) or
%                        -2 (apical)
...
%       - elecs:    vector that defines the electrodes from which to  
%                   stimulate i.e. 1:22 is all electrodes, and [4:7 10:20] 
%                   is electrodes 4-7 and 10-20
%       - e_levels: vector that indicates the electrode current levels in
%                   CU between 0 and 255. The standard is to set these to 
%                   MCL
%       - half:     (Optional) set to 1 to record the top half of the
%                   matrix where the maskers are basal to the probe or set
%                   to -1 to record the bottom half of the matrix where the
%                   maskers are apical to the probe. Otherwise set to 0 or
%                   exclude the input and NRT combinations for the full
%                   PECAP matrix will be written
%
%   Outputs:
%       - N/A
%       There are no direct outputs from this function, but running it
%       results in two .csv files that can be imported into Custom Sound EP
%       with the specified Neural Response Telemetry (NRT) parameters for
%       recording ECAPs for the PECAP method.
%
...
%   and masker conditions, but will use all available electrodes in the
%   1:22 range to record from.
%
%
%   Required Software:
%       - N/A
%   
%   Example Scripts:
%   For a full-matrix (SlowCAP) PECAP recorded basally to the probe - 
%   pecap_daq('Subject1', -2, 40, 98, 80, 50, [4:7 10:20], ...
%       [204 205 206 207 210 211 212 213 214 215 216 217 218 219 220], 0);
%   For a half-matrix (MediumCAP) PECAP recorded apically to the probe - 
%   pecap_daq('Subject1_half', +2, 40, 98, 80, 50, 1:22, 201:222, 1);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Developed: John Deeks, Amended: Charlotte Garcia                        %
% Updated: Charlotte Garcia, August 2021                                  %
...
Diag = 'on'; %switch to 'off' to record on only 1 side of the diagonal
...
if (min(electrodes) < ELECTRODE_MIN || max(electrodes) > ELECTRODE_MAX)
    fprintf(1, ...
        '%s: Electrode must be integer in range %d to %d - exiting.\n', ...
        strProgramName, ELECTRODE_MIN, ELECTRODE_MAX);
    return
end
if (phase_duration_us < PHASE_DUR_US_MIN || ...
        phase_duration_us > PHASE_DUR_US_MAX)
    fprintf(1, ...
        ['%s: Phase duration must be in range %.2f µs to %.2f µs - ' ...
        'exiting.\n'], strProgramName, PHASE_DUR_US_MIN, PHASE_DUR_US_MAX);
    return
end
if (min(e_levels) < LEVEL_CU_MIN || max(e_levels) > LEVEL_CU_MAX)
    fprintf(1, ...
        '%s: Level must be integers in range %d to %d - exiting.\n', ...
...
if length(Diag) == 2
    if min(Diag == 'on') == 1
        ...
        fprintf(1, ['%s: Diag input must either be specified as on or ' ...
            'off - exiting.\n'], strProgramName);
        return
    end
elseif length(Diag) == 3
    ...
    fprintf(1, ['%s: Diag input must either be specified as on or off ' ...
        '- exiting.\n'], strProgramName);
    return
end
...
if max_charge_nC > 212
    fprintf(1, ['%s: Charge must not exceed safety limit of 212 nC per' ...
        ' phase - exiting\n'], strProgramName);
    return;
end
if (interphase_gap_us < PHASEGAP_US_MIN || ...
        interphase_gap_us > PHASEGAP_US_MAX)
    fprintf(1, ['%s: Interphase gap duration must be in range %.2f µs' ...
        ' to %.2f µs - exiting.\n'], strProgramName, PHASEGAP_US_MIN, ...
        PHASEGAP_US_MAX);
    return
end
...
if ( ((max(electrodes)+Rec_Elec) > ELECTRODE_MAX) || ...
        ((max(electrodes)+Rec_Elec) > max_spec_electrode) )
    fprintf(1, ['%s: Recording electrode will be reset to -2 where ' ...
        'PROBE electrode > %d\n'], strProgramName, min(ELECTRODE_MAX, ...
        max_spec_electrode)-Rec_Elec);    
end
if ( ((min(electrodes)+Rec_Elec) < ELECTRODE_MIN) || ...
        ((min(electrodes)+Rec_Elec) < min_spec_electrode) )
    fprintf(1, ['%s: Recording electrode will be reset to +2 where ' ...
        'PROBE electrode < %d\n'], strProgramName, max(ELECTRODE_MIN, ...
        min_spec_electrode)-Rec_Elec);
end
if ~( (Gain_dB == GAINS(1)) || (Gain_dB == GAINS(2)) || ...
        (Gain_dB == GAINS(3)) || (Gain_dB == GAINS(4)) )
    fprintf(1, '%s: Gain_dB must be 40, 50, 60 or 70 dB\n', ...
        strProgramName);
    return;
end
if (Delay_us < MIN_DELAY_US || Delay_us > MAX_DELAY_US)
    fprintf(1, ['%s: Recording Delay must be in range %.2f µs to %.2f' ...
        ' µs - exiting.\n'], strProgramName, MIN_DELAY_US, MAX_DELAY_US);
    return
end

%% Summarise
fprintf(['\n-----\nNOTE: PLEASE READ THE DISCLAIMER IN THE HEADING ' ...
    'OF THIS PROGRAM PRIOR TO USE.\n-----\n\n'])
fprintf(1, ['Generating sequence for %d electrodes between %d and '...
...
fprintf(1, ['Probe rate = %2.0f Hz\t\t\tMasker-Probe Interval = ' ...
    '%2.0f us\n'], probe_rate_Hz, mpi_us);
fprintf(1, 'Gain is %d dB\t\t\t\tDelay is %d us\n', Gain_dB, Delay_us);
fprintf(1, 'Number of sweeps = %d\t\tNumber of samples = %d\n', ...
    n_sweeps, n_samples);
fprintf(1, ['Recording Active Electrode is %d relative to PROBE ' ...
    'electrode, %s mode\n\n'], Rec_Elec, rec_mode);

% add a specifciation saying if it's a mediumCAP matrix
if nargin > 8
    if half == 1
        fprintf('Generating a 1/2 PECAP matrix (mediumCAP)\n');
        fprintf('\tMaskers are basal to the probe\n\n');
    elseif half == -1
        fprintf('Generating a 1/2 PECAP matrix (mediumCAP)\n');
        fprintf('\tMaskers are apical to the probe\n\n');
    elseif half == 0
        fprintf('Generating a Full PECAP matrix (SlowCAP)\n');
        fprintf('\tMaskers are apical to the probe\n\n');
    end
elseif nargin == 8
    fprintf('Generating a Full PECAP matrix (SlowCAP)\n');
...
while p <= no_electrodes
    ...
    while n <= no_electrodes
        ...
        if str2double(a{2}) == str2double(a{19})
            % If we are recording from both sides along the diagonal
            if diag_rec == 1
                % record in the direction specified (within e1-22)
                if (electrodes(p) + rec_el) >= ELECTRODE_MIN && ...
                        (electrodes(p) + rec_el) <= ELECTRODE_MAX
        ...
        % If we are not on the diagonal    
            ...
            % if we don't have an indication of 'half' the recording matrix
            if nargin == 8
                a{1}=num2str(NRT);
                ...
                fprintf(fid,formatSpec,a{:});
                NRT = NRT + 1;
            % if we do have an input for 'half'
            elseif nargin > 8
                if half == 1
                    % if we are only recording mediumCAP aka half the PECAP
                    % matrix and have specified the top half where the
                    % maskers are basal to the probe
                    if electrodes(p) > electrodes(n)
                        a{1}=num2str(NRT);
                        % check to make sure that recording and masker 
                        % electrode are not the same - if so, move the  
                        % recording electrode between the probe and the 
                        % masker
                        if(electrodes(n) == (electrodes(p)+rec_el))
                            a{8}=num2str((electrodes(p)+electrodes(n))/2);
                        else
                ...
                elseif half == -1
                    % if we are only recording mediumCAP aka half the PECAP
                    % matrix and have specified the bottom half where the
                    % maskers are apical to the probe
                    if  electrodes(p) < electrodes(n)
                        a{1}=num2str(NRT);
                        % check to make sure that recording and masker 
                        % electrode are not the same - if so, move the  
                    ...
                    % check to make sure that recording and masker 
                    % electrode are not the same - if so, move the  
                    % recording electrode between the probe and the masker
                    if(electrodes(n) == (electrodes(p)+rec_el))
                        a{8}=num2str((electrodes(p)+electrodes(n))/2);
                    else
                        a{8}=num2str(electrodes(p)+rec_el);            
